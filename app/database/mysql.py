...
class MariaDBClient:
            ...
            async with conn.cursor() as cur:
                try:
                    ...
                    if query.upper().strip().startswith("INSERT"):
                        logger.debug(
                            f"Insert successful - lastrowid: {lastrowid}, rowcount: {rowcount}"
                        )
                        return {"id": lastrowid, "affected_rows": rowcount}
    ...
    async def exists(self, query: str, params: Optional[Tuple] = None) -> bool:
