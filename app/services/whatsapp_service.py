...
class MessageCache:
    ...
    def cleanup(self, max_age_minutes=30):
        ...
        to_remove = [key for key, timestamp in self.cache.items() if timestamp < cutoff]
...
class WhatsAppService:
    ...
    async def verify_webhook(self, mode: str, token: str, challenge: str) -> int:
        ...
        logger.info(
            f"Verifying webhook: mode={mode}, token={token}, challenge={challenge}, verify_token={verify_token}"
        )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    url, headers=self.openai_headers, files=files
                )
                ...
                for _ in range(max_retries):
                    status_response = await client.get(
                        f"{url}/{file_id}", headers=self.openai_headers
                    )
    ...
    async def process_webhook(self, request: WhatsAppWebhookRequest) -> Dict[str, Any]:
        try:
            ...
            organization = await self.organization_service.get_organization_by_phone_id(
                phone_id
            )
            ...
            try:
                ...
                message_time_readable = datetime.fromtimestamp(message_time).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
                ...
                if time_difference > 86400:  # 24 hours in seconds
                    ...
                    logger.warning(
                        f"Suspicious webhook payload: {json.dumps(request.model_dump(), default=str)}"
                    )
                ...
                log_whatsapp_message(
                    ...
                    message_type="error",
                    message_data={
                        "error": f"Error validating message timestamp: {str(e)}"
                    },
                    direction="system",
                )
            ...
            customer = await get_customer(
                messages[0].from_, contact.profile.name, organization_id
            )
            ...
            customer_context = (
                f"Customer: {contact.profile.name}, Phone: {messages[0].from_}"
            )
            ...
            for message in messages:
                if message.type == "text":
                    content_items.append({"type": "text", "text": message.text.body})
                    ...
                    try:
                        ...
                        file_response = await self.upload_file(
                            image_data, f"whatsapp_image_{message.image.id}.jpg"
                        )

                        if not file_response or "id" not in file_response:
                            raise ValueError(
                                "Failed to get valid file response from OpenAI"
                            )
                        logger.info(
                            f"File uploaded successfully: {file_response['id']}"
                        )
            ...
            org_phones = await self.organization_service.get_organization_phones(
                str(organization["id"])
            )
            ...
            if (conversation and conversation.mode == "human") or is_admin:
                logger.info(
                    f"Customer {messages[0].from_} is in human mode or is admin - skipping AI processing"
                )

                if not conversation:
                    thread = await self.assistant_service.create_thread()
                    conversation = await insert_conversation(
                        id=thread.thread_id, contact_id=customer.id, mode="human"
                    )
            ...
            # TODO: To be replaced with API KEY
            # Ensure organization has an agent ID
            if not organization["agent_id"]:
                return {"status": "error", "message": "Organization agent ID not found"}

            logger.info(f"Getting AI response {conversation.id} : {content_items}")
            # Get AI response with all message contents and metadata
            chat_response = await self.assistant_service.chat(
                ChatRequest(
                    assistant_id=organization["agent_id"],
                    thread_id=conversation.id if conversation else None,
                    messages=[ChatMessage(role="user", content=content_items)],
                )
            )
            ...
            if not conversation:
                conversation = await insert_conversation(
                    id=chat_response.thread_id, contact_id=customer.id, mode="ai"
                )
            ...
            await insert_message(
                conversation_id=conversation.id, contents=content_items, role="user"
            )
            ...
            for content in content_items:
                ...
                await manager.broadcast(message, conversation.id)
            ...
            if chat_response.messages:
                ...
                if assistant_message and assistant_message.content:
                    ...
                    if response_text:
                        logger.info(
                            f"Sending response to {messages[0].from_}: {response_text}"
                        )
                        ...
                        res = await insert_message(
                            conversation_id=conversation.id,
                            contents=[{"text": response_text, "type": "text"}],
                            role="assistant",
                        )
                        ...
                        log_whatsapp_message(
                            ...
                            message_data={
                                ...
                                "debug": {
                                    ...
                                    "has_assistant_message": assistant_message
                                    is not None,
                                    "content_type": type(
                                    ...
                                    "content_value": str(assistant_message.content)
                                    if assistant_message
                                    else None,
                                },
                            },
                    ...
                    log_whatsapp_message(
                        ...
                        message_data={
                            ...
                            "debug": {
                                "status": chat_response.status,
                                "messages_count": len(chat_response.messages)
                                if chat_response.messages
                                else 0,
                                "has_assistant_message": assistant_message is not None,
                                "message_roles": [
                                    msg.role for msg in chat_response.messages
                                ]
                                if chat_response.messages
                                else [],
    ...
    async def send_message(
        self, to: str, message: str, phone_id: str
    ) -> Dict[str, Any]:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    log_whatsapp_message(
                        ...
                        message_type="error",
                        message_data={
                            "error": f"Error sending message: {response.text}"
                        },
                        direction="system",
                    )
