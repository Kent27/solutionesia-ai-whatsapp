...
class OrganizationService:
    ...
    async def get_organization_human_conversations(
        ...
        try:
            ...
            logger.error(
                f"Error getting organization human conversations: {str(e)}",
                exc_info=True,
            )
            raise

    async def get_organization_conversations(
        self, org_id: str, filter: ConversationFilter
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error getting organization conversations: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_organization_contacts(
        ...
        try:
            ...
            # 2. Get contacts
            contacts_query = """
                SELECT id, name, phone_number, email, stamps, organization_id, created_at, updated_at
                FROM contacts 
                WHERE organization_id = %s
                LIMIT %s OFFSET %s
            """
            contacts_rows = await self.db.fetch_all(
                contacts_query, (org_id, limit, offset)
            )

            contacts = []
            contact_ids = [str(row[0]) for row in contacts_rows]
            ...
            if contact_ids:
                ...
                """
                labels_rows = await self.db.fetch_all(labels_query, tuple(contact_ids))

                for lr in labels_rows:
                    cid = str(lr[0])
                    if cid not in labels_map:
                        labels_map[cid] = []
                    labels_map[cid].append(
                        {"id": str(lr[1]), "name": lr[2], "color": lr[3]}
                    )
            ...
            logger.error(
                f"Error getting organization contacts: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_contact_conversations(
        ...
        try:
            ...
            check_query = (
                "SELECT id FROM contacts WHERE id = %s AND organization_id = %s"
            )
            ...
            logger.error(
                f"Error getting contact conversations: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_organization_by_id(
        self, id: str
    ) -> Optional[GetOrganizationResponse]:
    ...
    async def get_organization_by_phone_id(
        self, phone_id: str
    ) -> Optional[GetOrganizationResponse]:
    ...
    async def update_status(
        self, org_id: str, status_data: OrganizationUpdateStatus
        ...
        try:
            ...
            # Fetch updated
            query_get = (
                "SELECT id, name, email, status FROM organizations WHERE id = %s"
            )
    ...
    async def update_profile(
        self, org_id: str, profile_data: OrganizationUpdateProfile
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error updating organization profile: {str(e)}", exc_info=True
            )
            raise
